...
# 导入HackerNews客户端类，爬取HackerNews top stories, 生成进展简报
...
def github_job(subscription_manager, github_client, report_generator, notifier, days):
    ...
    for repo in subscriptions:
        # 遍历每个订阅的仓库，执行以下操作
        markdown_file_path = github_client.export_progress_by_date_range(
            repo, days)
        # 从Markdown文件自动生成进展简报
        report, report_file_path = report_generator.generate_report_by_date_range(
            markdown_file_path, days)
...
def hackernews_job(hackernews_client, report_generator, notifier, days):
    ...
    # 从Markdown文件自动生成进展简报
    markdown_file_path = hackernews_client.export_progress_by_date_range(days)
    report, report_file_path = report_generator.generate_hackernews_report(
        markdown_file_path)
    notifier.notify("hackernews", report)
    LOG.info(f"[hackernews定时任务执行完毕]")
...
def main():
    ...
    subscription_manager = SubscriptionManager(
        config.subscriptions_file)  # 创建订阅管理器实例

    # 启动时立即执行（如不需要可注释）
    github_job(subscription_manager, github_client,
               report_generator, notifier, config.freq_days)
    hackernews_job(hackernews_client, report_generator,
                   notifier, config.hackernews_freq_days)
    ...
    ).do(hackernews_job, hackernews_client, report_generator, notifier, config.hackernews_freq_days)
