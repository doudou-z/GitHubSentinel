...
def export_progress_by_date_range(repo, days):
    ...
    report, report_file_path = report_generator.generate_report_by_date_range(
        raw_file_path, days)  # 生成并获取报告内容及文件路径
...
def export_hackernews_by_date_range(days):
    # 定义一个函数，用于导出和生成指定时间范围内项目的进展报告
    raw_file_path = hacker_news_client.export_progress_by_date_range(
        days)  # 导出原始数据文件路径
...
export_report = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(value=2, minimum=1, maximum=7, step=1,
                  label="报告周期", info="生成项目过去一段时间进展，单位：天"),
        # 滑动条选择报告的时间范围
    ],
...
export_hacker_report = gr.Interface(
    ...
    outputs=[gr.Markdown(), gr.File(label="下载Hackernews报告")
             ],  # 输出格式：Markdown文本和文件下载
)

demo = gr.TabbedInterface([export_report, export_hacker_report],
                          ["Generate Report", "Generate HackerNews"])
